...
async function startCheckout() {
  ...
  try {
    const meRes = await fetch('/api/me', { headers: apiHeaders() });
    const me = await meRes.json();
    if (me.pro) { toast("You're already Pro — nothing to upgrade!"); return; }
  } catch (err) { /* if this check fails, fall through to checkout anyway */ }
  try {
